...
int main()
{
	// the fact that arrays count 0 as the first space is so gad dang annoying
	int odds[10];
	int player1[5]; 
	int player2[5];
	...
	int count;
	int winner,choice;
	char raceAgain = 'K';//the "K" is just to initialize the variable
    string human, human2;
    string computer;
    ...
    //this is so if youre lonely you can bet against a computer that randomly generates all of the variables
        cout<<"If you would like to play against a computer hit 1 and if you have another player hit 2\n";
        cin>>choice;
    ...
    //this way it calls the function so the computer can randomly generate stuff, but the human still does everything as if wer in the normal player vs player situation
        if (choice==1)
        {
            cout<<"What is the your first name player 1?\n";
            cin>>human;
    ...
    //this is just player vs player and goes on the same perameters for both
        if (choice==2)
        {
            ...
            cout<<"What is the your first name player 2?\n";
            cin>>human2;
            moneyAmount(player1, human);
    ...
    //this is what causes the program to exit
    catch (int e)
    {
        cout<<"That is not a valid entry. The program will now exit.\n";
        exit(1);
    }
        cout<<"\n\n";
        menu();
	...
	do
	{
        ...
        {
            cout<<"Sorry one, or both of you has reached $0 and can no longer bet, time to exit. Thanks for playing!\n";
			raceAgain = 'n';
        }
